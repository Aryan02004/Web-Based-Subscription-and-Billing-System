...
export default function PublicCheckout() {
  ...
  useEffect(() => {
    async function loadCheckoutPage() {
      ...
      try {
        ...
        setPageError(
          error?.message ||
            "Unable to load checkout details. Please verify the link.",
        );
      } finally {
        setLoading(false);
      }
  ...
  const cardPlans = useMemo(
    () =>
      plans.map((plan) => ({
        ...
        isPopular: plan.mostPopular === true,
      })),
    [plans],
  );
  ...
  const startCheckout = async () => {
    ...
    try {
      if (!window.Razorpay) {
        throw new Error(
          "Razorpay checkout script not loaded. Refresh the page and try again.",
        );
      }
      ...
      const options = {
        ...
        description:
          selectedPlan.planName || selectedPlan.name || "Subscription plan",
        order_id: order.id,
        prefill: {
          name: fullName,
  ...
  const logoColor =
    placeholderLogoColors[
      organization?.name
        ? organization.name.length % placeholderLogoColors.length
        : 0
    ];
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                <div
                  className={`flex h-16 w-16 items-center justify-center rounded-3xl bg-gradient-to-br ${logoColor} text-2xl font-semibold text-white shadow-lg shadow-slate-200/40`}
                >
              ...
              <p className="max-w-2xl text-base leading-8 text-slate-600">
                Choose the best subscription plan for you and
                ...
                <div className="mb-5 flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-2 text-base text-slate-600">
                      All active plans available for {organization.name}.
                    </p>
                  </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <p className="mt-5 text-sm leading-7 text-slate-600">
                          {plan.description ||
                            "A flexible plan for your organization."}
                        </p>
                        ...
                        <div className="mt-6 flex items-center justify-between gap-3 text-sm">
                          <span
                            className={`rounded-full px-3 py-1 font-semibold ${isSelected ? "bg-cyan-500 text-white" : "bg-slate-100 text-slate-700"}`}
                          >
                            {isSelected ? "Selected" : "Select plan"}
                          </span>
                          <span
                            className={`rounded-full px-3 py-1 text-xs ${plan.active === false ? "bg-red-100 text-red-700" : "bg-emerald-100 text-emerald-700"}`}
                          >
            ...
            <aside className="space-y-6">
              <div className="rounded-[2rem] border border-slate-200 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-800 p-6 shadow-xl shadow-slate-900/10 text-white">
                ...
                <p className="mt-3 text-sm leading-7 text-slate-200">
                  {organization.industry || "Business plan"} • Secure checkout
                  experience
                </p>
                <div className="mt-6 rounded-[1.75rem] bg-slate-900/70 p-4">
                  ...
                  <p className="mt-3 text-sm text-slate-200">
                    Customer details are persisted securely and the payment is
                    processed via Razorpay. This link works for all active
                    plans.
                  </p>
                </div>
              </div>
                ...
                <div className="mb-5 flex items-center justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-500">
                      Enter contact info for billing and support.
                    </p>
                  </div>
                </div>
                <div className="space-y-4">
                ...
                <div className="mt-5 space-y-4">
                  <div className="rounded-[1.75rem] bg-slate-50 p-5">
                    ...
                    <p className="mt-3 text-sm text-slate-600">
                      {selectedPlan?.description ||
                        "Pick the best plan to complete checkout."}
                    </p>
                  </div>
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div className="rounded-3xl bg-slate-50 p-4">
                      ...
                      <span className="text-sm text-slate-500">This Amount is excludes GST. 18% GST will be charged at the time of payment.</span>
                    </div>
                    <div className="rounded-3xl bg-slate-50 p-4">
                      <p className="text-xs uppercase tracking-[0.24em] text-slate-500">
                        Organization
